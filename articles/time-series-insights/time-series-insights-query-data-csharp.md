---
title: "données aaaQuery hello en environnement de Azure Insights de série de temps à l’aide de C# | Documents Microsoft"
description: "Ce didacticiel décrit comment tooquery les données à partir de hello environnement Insights de série de temps à l’aide de c#, par exemple de code."
keywords: 
services: tsi
documentationcenter: 
author: ankryach
...
---
# <a name="query-data-from-hello-azure-time-series-insights-environment-using-c"></a><span data-ttu-id="5da54-103">Interroger des données à partir de l’environnement de Azure temps série Insights hello à l’aide de c#</span><span class="sxs-lookup"><span data-stu-id="5da54-103">Query data from hello Azure Time Series Insights environment using C#</span></span>

<span data-ttu-id="5da54-104">Cet exemple c# montre comment tooquery des données à partir de hello environnement Azure temps série Insights.</span><span class="sxs-lookup"><span data-stu-id="5da54-104">This C# example demonstrates how tooquery data from hello Azure Time Series Insights environment.</span></span>
<span data-ttu-id="5da54-105">exemple Hello montre plusieurs exemples simples d’utilisation de l’API de requête :</span><span class="sxs-lookup"><span data-stu-id="5da54-105">hello sample shows several basic examples of Query API usage:</span></span>
1. <span data-ttu-id="5da54-106">Comme une étape de préparation, obtenir le jeton d’accès de hello via hello API Azure Active Directory.</span><span class="sxs-lookup"><span data-stu-id="5da54-106">As a preparation step, acquire hello access token through hello Azure Active Directory API.</span></span> <span data-ttu-id="5da54-107">Passez ce jeton Bonjour `Authorization` en-tête de chaque demande de requête API.</span><span class="sxs-lookup"><span data-stu-id="5da54-107">Pass this token in hello `Authorization` header of every Query API request.</span></span> <span data-ttu-id="5da54-108">Pour configurer les applications non interactives, voir [Authentification et autorisation](time-series-insights-authentication-and-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="5da54-108">For setting up non-interactive applications, see [Authentication and authorization](time-series-insights-authentication-and-authorization.md).</span></span> <span data-ttu-id="5da54-109">Assurez-vous en outre, toutes les constantes hello définies au début de hello d’exemple hello sont correctement définies.</span><span class="sxs-lookup"><span data-stu-id="5da54-109">Also, ensure all hello constants defined at hello beginning of hello sample are correctly set.</span></span>
2. <span data-ttu-id="5da54-110">liste de Hello des environnements qui hello utilisateur a toois accès obtenu.</span><span class="sxs-lookup"><span data-stu-id="5da54-110">hello list of environments that hello user has access toois obtained.</span></span> <span data-ttu-id="5da54-111">Un des environnements de hello est récupéré en tant qu’environnement hello dignes d’intérêt, et plus les données sont interrogées pour cet environnement.</span><span class="sxs-lookup"><span data-stu-id="5da54-111">One of hello environments is picked up as hello environment of interest, and further data is queried for this environment.</span></span>
3. <span data-ttu-id="5da54-112">Par exemple de demande HTTPS, les données de disponibilité sont demandées pour environnement hello dignes d’intérêt.</span><span class="sxs-lookup"><span data-stu-id="5da54-112">As an example of HTTPS request, availability data is requested for hello environment of interest.</span></span>
4. <span data-ttu-id="5da54-113">Comme un exemple de requête WebSocket, les données d’événement agrégats sont demandées pour environnement hello dignes d’intérêt.</span><span class="sxs-lookup"><span data-stu-id="5da54-113">As an example of web socket request, event aggregates data is requested for hello environment of interest.</span></span> <span data-ttu-id="5da54-114">Données sont demandées pour la plage de temps de disponibilité ensemble hello.</span><span class="sxs-lookup"><span data-stu-id="5da54-114">Data is requested for hello whole availability time range.</span></span>

## <a name="c-example"></a><span data-ttu-id="5da54-115">Exemple en code C#</span><span class="sxs-lookup"><span data-stu-id="5da54-115">C# example</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="5da54-116">Étapes suivantes</span><span class="sxs-lookup"><span data-stu-id="5da54-116">Next steps</span></span>

<span data-ttu-id="5da54-117">Pour la référence d’API de requête complète hello, consultez hello [API de requête](/rest/api/time-series-insights/time-series-insights-reference-queryapi) document.</span><span class="sxs-lookup"><span data-stu-id="5da54-117">For hello full Query API reference, see hello [Query API](/rest/api/time-series-insights/time-series-insights-reference-queryapi) document.</span></span>
