...
---
# <a name="use-cli-toomonitor-and-scale-a-single-sql-database"></a>Utilisez CLI toomonitor et mettre à l’échelle d’une base de données SQL

Cet exemple de script CLI d’Azure met à l’échelle un niveau de performance différents de tooa de base de données SQL Azure unique après interrogation des informations sur la taille de base de données hello hello. 
...
| [az sql db show-usage](https://docs.microsoft.com/cli/azure/sql/db#show-usage) | Affiche des informations sur l’utilisation de taille hello pour une base de données. |
| [az sql db update](https://docs.microsoft.com/cli/azure/sql/db#update) | Met à jour les propriétés de base de données (par exemple, le niveau niveau ou des performances du service hello) ou déplace une base de données en, d’ou entre les pools élastiques. |
