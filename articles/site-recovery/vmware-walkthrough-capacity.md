...
---
# <a name="step-3-plan-capacity-and-scaling-for-vmware-tooazure-replication"></a>Étape 3 : Planifier la capacité et la mise à l’échelle pour la réplication tooAzure VMware
...
1. Télécharger hello [outil de planification du déploiement](https://aka.ms/asr-deployment-planner) pour la réplication de VMware.
2. [Lisez cet article](site-recovery-deployment-planner.md) toounderstand comment toorun hello outil.
3. À l’aide d’outil de hello vous rassemblez des informations sur les ordinateurs virtuels compatibles et incompatibles, les disques par machine virtuelle, et données évolution par disque. outil de Hello couvre également les besoins en bande passante et hello Azure infrastructure nécessaire pour le basculement de réplication et de test réussi.
...
Suivez ces tooset obtenir des instructions d’un serveur de processus supplémentaires. Après avoir configuré le serveur de hello, vous migrez source machines toouse il.

1. Dans **serveurs de récupération de Site**, cliquez sur le serveur de configuration hello > **+ serveur de processus**.
...
3. Téléchargez le fichier du programme d’installation de Site Recovery unifiée hello.
4. Exécutez le programme d’installation tooinstall hello processus serveur et l’inscrire dans le coffre hello.
5. Dans **avant de commencer**, sélectionnez **ajouter tooscale de serveurs de processus supplémentaire déploiement**.
6. Dans **détails du serveur de Configuration**, spécifiez l’adresse IP de hello hello du serveur de configuration et hello phrase secrète. Si vous n’avez pas hello le mot de passe, l’obtenir en exécutant **[SiteRecoveryInstallationFolder]\home\sysystems\bin\genpassphrase.exe – n** sur le serveur de configuration hello.

    ![Serveur de configuration](./media/vmware-walkthrough-capacity/add-ps2.png)
7. Exécutez le programme d’installation Bonjour hello restantes même façon que lorsque vous configurez le serveur de configuration hello.

### <a name="migrate-machines-toouse-hello-process-server"></a>Migration d’ordinateurs toouse hello processus serveur
