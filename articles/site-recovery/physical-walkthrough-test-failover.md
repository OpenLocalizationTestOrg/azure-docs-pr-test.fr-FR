...
---
# <a name="step-11-run-a-test-failover-of-physical-servers-tooazure"></a>Étape 11 : Exécuter un test de basculement de serveurs physiques tooAzure

Cet article décrit comment toorun un test de basculement à partir de local tooAzure serveurs physiques, à l’aide de hello [Azure Site Recovery](site-recovery-overview.md) service Bonjour portail Azure.
...
Avant d’exécuter un test de basculement que nous vous recommandons de vérifier les propriétés du serveur hello et d’apporter des modifications vous devez. Vous pouvez accéder à des propriétés de machine virtuelle hello dans **éléments répliqués**. Hello **Essentials** panneau affiche les informations d’état et les paramètres de l’ordinateur.
...
- Lorsque vous activez la protection pour un serveur, compte de stockage tooa réplique les données des ordinateurs virtuels. Disques gérés sont créés et attaché toohello VM uniquement en cas de basculement.
- Disques gérés peuvent être créés uniquement pour les machines virtuelles de Azure déployé à l’aide du modèle de gestionnaire de ressources hello.  
...
Nous vous conseillons de vérifier les propriétés hello du serveur de source de hello avant d’exécuter un basculement.

1. Dans **éléments protégés**, cliquez sur **répliquées des éléments**et sur l’ordinateur hello.
2. Bonjour **répliqué élément** volet, vous pouvez voir un résumé des informations sur l’ordinateur, l’état d’intégrité et points de récupération disponibles hello plus récente. Cliquez sur **propriétés** tooview plus en détail.
...
1. toofail sur un seul ordinateur, dans **paramètres** > **répliquées des éléments**, cliquez sur un ordinateur hello > **+ Test de basculement** icône.
...
4. Cliquez sur **OK** toobegin hello basculement. Vous pouvez suivre la progression en cliquant sur hello machine tooopen ses propriétés, ou sur hello **Test de basculement** travail dans le nom du coffre > **paramètres** > **travaux**  >  **Les tâches de récupération de site**.

5. Une fois hello basculement terminé, vous devez également être toosee en mesure de réplica de hello Azure VM apparaissent dans hello portail Azure > **virtuels**. Assurez-vous que VM hello est hello approprié, qui sa connexion réseau approprié de toohello, et qu’il s’exécute.
...
1. Une fois que vous avez terminé, cliquez sur **basculement de test de nettoyage** sur l’ordinateur ou le plan de récupération hello.
2. Dans **Notes**, enregistrer et enregistrer toutes les observations associées au basculement de test hello.
3. action de nettoyage Hello supprime des machines virtuelles Azure qui ont été créés pendant le test de basculement.

## <a name="summary"></a>Résumé

Si le basculement de test hello est terminé avec succès, répliquent vos serveurs physiques et que vous avez vérifié qu’ils peuvent basculer tooAzure. Maintenant, vous pouvez exécuter des basculements conformément aux exigences de votre organisation. 

N’oubliez pas que vous ne peuvent pas basculer actuellement Azure tooa les serveur physique. Vous avez toofail sauvegarder tooa VMware VM. Cela signifie que vous avez besoin d’une infrastructure VMware en local dans arrière toofail de commande. [En savoir plus](site-recovery-failback-azure-to-vmware.md) sur l’échec précédent tooVMware de machines virtuelles Azure.
