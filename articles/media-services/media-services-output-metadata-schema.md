...
Un travail d’encodage est associé à un élément multimédia d’entrée (ou actifs) sur lequel vous souhaitez tooperform certaines tâches d’encodage. Par exemple, pour encoder un fichier MP4 tooH.264 MP4 des fichiers à débit adaptatif définit ; créer une miniature ; créer des superpositions. À l’achèvement d’une tâche, une ressource de sortie est générée.  la ressource en sortie Hello contient vidéo, audio, miniatures, etc. hello ressource en sortie contient également un fichier contenant des métadonnées sur l’élément multimédia de sortie hello. nom du fichier XML de métadonnées hello Hello a hello suivant le format : &lt;nom_fichier_source&gt;_manifest.xml (par exemple, BigBuckBunny_manifest.xml).  

Si vous souhaitez que le fichier de métadonnées tooexamine hello, vous pouvez créer un **SAS** recherche et le téléchargement hello ordinateur local tooyour de fichier.  

Cette rubrique décrit les éléments de hello et types de schéma XML de hello sur la résultat de hello des métadonnées (&lt;nom_fichier_source&gt;_manifest.xml) est basé. Pour plus d’informations sur le fichier hello qui contient des métadonnées sur l’élément multimédia d’entrée de hello, consultez [d’entrée de métadonnées](media-services-input-metadata-schema.md).  

> [!NOTE]
> Vous pouvez trouver le code du schéma complet hello et exemple de code XML à la fin de hello de cette rubrique.  
>
>

## <a name="AssetFiles "></a> Élément racine AssetFiles
Collection d’entrées AssetFile pour le travail d’encodage hello.  

### <a name="child-elements"></a>Éléments enfants
| Nom | Description |
| --- | --- |
| **AssetFile**<br/><br/> minOccurs="0" maxOccurs="1" |Un [élément AssetFile](media-services-output-metadata-schema.md) qui fait partie de hello collection AssetFiles. |
...
| --- | --- | --- |
| **Name**<br/><br/> Requis |**xs:string** |Hello nom du fichier multimédia. |
| **Taille**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:long** |Taille de fichier d’élément multimédia hello en octets. |
| **Durée**<br/><br/> Requis |**xs:duration** |Durée de lecture du contenu. |

### <a name="child-elements"></a>Éléments enfants
| Nom | Description |
| --- | --- |
| **Sources** |Collection de fichiers de média d’entrée/source, qui a été traitée dans commande tooproduce cet AssetFile. Pour plus d'informations, consultez la page [Élément source](media-services-output-metadata-schema.md). |
| **VideoTracks**<br/><br/> minOccurs="0" maxOccurs="1" |Chaque élément AssetFile physique peut contenir zéro ou plusieurs pistes vidéo entrelacées dans un format de conteneur approprié. Il s’agit de collection hello de toutes ces pistes vidéos. Pour plus d’informations, consultez [élément VideoTracks](media-services-output-metadata-schema.md). |
| **AudioTracks**<br/><br/> minOccurs="0" maxOccurs="1" |Chaque élément AssetFile physique peut contenir zéro ou plusieurs pistes audio entrelacées dans un format de conteneur approprié. Il s’agit de collection hello de toutes ces pistes audio. Pour plus d’informations, consultez [élément AudioTracks](media-services-output-metadata-schema.md). |

## <a name="Sources "></a> Sources (élément)
Collection de fichiers de média d’entrée/source, qui a été traitée dans commande tooproduce cet AssetFile.  
...
Chaque élément AssetFile physique peut contenir zéro ou plusieurs pistes vidéo entrelacées dans un format de conteneur approprié. Il s’agit de collection hello de toutes ces pistes vidéos.  
...
| Nom | Description |
| --- | --- |
| **VideoTrack**<br/><br/> minOccurs="1" maxOccurs="unbounded" |Une piste vidéo spécifique dans l’élément AssetFile parent de hello. Pour plus d’informations, consultez [élément VideoTrack](media-services-output-metadata-schema.md#VideoTrack). |

## <a name="VideoTrack"></a> Élément VideoTrack
Une piste vidéo spécifique dans l’élément AssetFile parent de hello.  
...
| Nom | Type | Description |
| --- | --- | --- |
| **Id**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:int** |Index de base zéro de cette piste vidéo. **Remarque :** cela n’est pas nécessairement hello trackid tel qu’utilisé dans un fichier MP4. |
| **FourCC**<br/><br/> Requis |**xs:string** |Code FourCC du codec vidéo. |
| **Profil** |**xs:string** |Profil h264 (uniquement applicable tooH264 codec). |
| **Niveau** |**xs:string** |Niveau h264 (uniquement applicable tooH264 codec). |
...
| **Bitrate**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:int** |Débit binaire vidéo moyen en kilobits par seconde, tel que calculé à partir de hello AssetFile. Compte seule charge utile de flux élémentaire hello et n’inclut pas de surcharge de packaging hello. |
| **TargetBitrate**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:int** |Cibler le débit binaire moyen pour cette piste vidéo, tel que demandé via hello valeur prédéfinie d’encodage, en kilobits par seconde. |
...
Chaque élément AssetFile physique peut contenir zéro ou plusieurs pistes audio entrelacées dans un format de conteneur approprié. Il s’agit de collection hello de toutes ces pistes audio.  
...
| Nom | Description |
| --- | --- |
| **AudioTrack**<br/><br/> minOccurs="1" maxOccurs="unbounded" |Une piste audio spécifique dans l’élément AssetFile parent de hello. Pour plus d’informations, consultez [élément AudioTrack](media-services-output-metadata-schema.md). |

## <a name="AudioTrack "></a> Élément AudioTrack
Une piste audio spécifique dans l’élément AssetFile parent de hello.  
...
| Nom | Type | Description |
| --- | --- | --- |
| **Id**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:int** |Index de base zéro de cette piste audio. **Remarque :** cela n’est pas nécessairement hello trackid tel qu’utilisé dans un fichier MP4. |
...
| **Bitrate**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:int** |Débit binaire audio moyen en bits par seconde, tel que calculé à partir de hello AssetFile. Compte seule charge utile de flux élémentaire hello et n’inclut pas de surcharge de packaging hello. |
| **BitsPerSample**<br/><br/> minInclusive ="0"<br/><br/> Requis |**xs:int** |Type de bits par échantillon pour le format wFormatTag hello. |
...
| **IntegratedLoudnessSpeechPercentage**<br/><br/> minInclusive ="0" maxInclusive="100" |**xs:float** |Contenu vocal via le programme hello, sous forme de pourcentage. |
...
## <a name="xml"></a> Exemple XML
 Hello Voici un exemple hello métadonnées du fichier de sortie.  
