...
---
# <a name="examples-of-mapping-an-existing-web-service-tooodata-through-csdls"></a>Exemples de mappage existant web tooOData service via CSDLs
> [!IMPORTANT]
...
Utiliser toocreate de données brutes de valider un subordonné de nouveau et son serveur URL(location) ou tooupdate partie hello secondaire sur le serveur de hello définie par l’URL de retour.  Où hello subordonné est un flux de données, c'est-à-dire non structurée, par ex. un fichier texte.  Prenez garde que POST ne soit pas idempotent sans emplacement.
...
Utilisez supprimer tooremove un URI spécifié.
...
Utiliser toocreate de données brutes de valider un subordonné de nouveau et son serveur URL(location) ou tooupdate partie hello secondaire sur le serveur de hello définie par l’URL de retour.  Où hello subordonné est une structure. Prenez garde que POST ne soit pas sans emplacement.
...
Utilisez PUT toocreate subordonné nouveau ou subordonné entière de hello tooupdate sur un serveur défini par l’URL.  Hello subordonné est une structure, PUT est idempotente plusieurs occurrences aboutira alors à hello même état, c'est-à-dire par exemple, x=5.  Put doit être utilisé avec hello contenu complètes de hello spécifié ressource.
...
Utilisez PUT brutes toocreate données subordonné nouveau ou subordonné entière de hello tooupdate à une URL de serveur défini.  Où hello subordonné est un flux de données, c'est-à-dire non structurée, par ex. un fichier texte.  PUT est idempotente plusieurs occurrences aboutira alors à hello même état, c'est-à-dire par exemple, x=5.  Put doit être utilisé avec hello contenu complètes de hello spécifié ressource.
...
Utilisez obtenir brutes données tooreturn subordonné qui n’est pas structuré, par exemple, le texte.
...
Utilisez la pagination RESTful d’implémentation via vos données avec GET.  Pagination par défaut est définie de ligne too100 par page de données.
