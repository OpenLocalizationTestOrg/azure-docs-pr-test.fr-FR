...
* Créer un jeu hello de comptes de stockage à l’aide de votre convention d’affectation de noms. Vous pouvez utiliser le portail Azure CLI ou hello de hello.
...
Les disques de système d’exploitation et disques de données ont une taille maximale de 4 To. Vous pouvez utiliser le Gestionnaire de Volume logique (LVM) toosurpass cette limite en regroupant les disques toopresent logique des volumes de données supérieure à 1 023 Go tooyour machine virtuelle.
...
En plus de vous toocreate disques supérieure à 1 023 Go, dans de nombreux cas, à l’aide de la répartition des disques de données améliore les performances en permettant à plusieurs objets BLOB de stockage de hello tooback pour un volume unique. L’agrégation par bandes, hello d’e/s requis toowrite et les données lues à partir d’un seul disque logique se déroule en parallèle.

Azure impose des limites de nombre de hello de disques de données et de la quantité de bande passante disponible, en fonction de la taille de machine virtuelle hello. Pour en savoir plus, consultez [Tailles de machines virtuelles](sizes.md).
