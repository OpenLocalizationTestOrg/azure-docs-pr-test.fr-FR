...
Ne créez pas plusieurs disques gérés identiques à partir d’un fichier de disque dur virtuel dans un court laps de temps. toocreate des disques gérés à partir d’un fichier de disque dur virtuel, instantané d’objet blob du fichier de disque dur virtuel hello est créé et il est utilisé toocreate géré disques. Instantané d’objet seul blob peut être créé dans une minute qui provoque l’échec de la création de disque toothrottling échéance. tooavoid cette limitation, créer un [instantané managé à partir du fichier de disque dur virtuel hello](virtual-machines-windows-powershell-sample-create-snapshot-from-vhd.md?toc=%2fpowershell%2fmodule%2ftoc.json) , puis utilisez hello gérés instantané toocreate plusieurs disques gérés dans peu de temps. 
