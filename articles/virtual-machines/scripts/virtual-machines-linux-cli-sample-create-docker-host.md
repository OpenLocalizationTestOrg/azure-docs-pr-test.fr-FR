...
Ce script crée une machine virtuelle avec Docker activé et démarre un conteneur Docker exécutant NGINX. Après avoir exécuté le script de hello, vous pouvez accéder serveur de web NGINX hello via hello nom de domaine complet de hello machine virtuelle Azure. 
...
| [azure vm extension set](https://docs.microsoft.com/cli/azure/vm/extension#set) | Ajoute et exécute un tooa d’extension de machine virtuelle machine virtuelle. Dans cet exemple, hello extension de machine virtuelle de Docker est tooconfigure utilisé un hôte Docker.|
