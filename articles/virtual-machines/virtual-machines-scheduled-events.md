...
> [!NOTE] 
> Aperçus deviennent tooyou disponible sur condition hello que vous acceptez les conditions d’utilisation de toohello. Pour plus d’informations, consultez [Conditions d’Utilisation Supplémentaires relatives aux Évaluations Microsoft Azure](https://azure.microsoft.com/en-us/support/legal/preview-supplemental-terms/).
>
...
## <a name="scheduled-events---hello-basics"></a>Événements planifiés - hello notions de base  
...
### <a name="discovering-hello-endpoint"></a>Hello point de terminaison de découverte
...
Maintenance des ordinateurs virtuels via hello portail Azure, API, CLI, initiée par l’utilisateur ou de PowerShell entraîne des événements planifiés. Cela vous permet de logique de préparation maintenance tootest hello dans votre application et tooprepare de votre application pour une maintenance initiée par l’utilisateur.
...
| EventType | Impact provoqué par cet événement. <br><br> Valeurs : <br><ul><li> `Freeze`: hello Machine virtuelle est planifiée toopause pendant quelques secondes. Hello du processeur est suspendu, mais il n’existe aucun impact sur la mémoire, les fichiers ouverts ou les connexions réseau. <li>`Reboot`: hello Machine virtuelle est planifiée pour le redémarrage (mémoire non persistant est perdu). <li>`Redeploy`: hello Machine virtuelle est planifiée toomove tooanother nœud (disques éphémères sont perdues). |
| ResourceType | Type de ressource affecté par cet événement. <br><br> Valeurs : <ul><li>`VirtualMachine`|
| Ressources| Liste des ressources affectées par cet événement. Cela est assurée au plus un toocontain machines [domaine de mise à jour](windows/manage-availability.md), mais ne peut ne pas contenir toutes les machines de hello UD. <br><br> Exemple : <br><ul><li> ["FrontEnd_IN_0", "BackEnd_IN_0"] |
...
- En savoir plus sur les API disponibles dans hello de hello [service de métadonnées de l’instance](virtual-machines-instancemetadataservice-overview.md).
