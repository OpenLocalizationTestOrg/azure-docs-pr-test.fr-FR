...
En choisissant Microsoft Azure comme partenaire SAP cloud prêt, vous serez en mesure de tooreliably exécuter votre SAP charges de travail critiques sur un scaaleable, conforme et la plateforme d’entreprise éprouvées.  Évolutivité hello, une flexibilité, d’obtenir et d’économies d’Azure. Avec hello développé le partenariat entre Microsoft et SAP, vous pouvez exécuter des applications SAP sur les scénarios de développement et de test et de production dans azure - et entièrement pris en charge. À partir de SAP NetWeaver tooSAP S4/HANA Linux tooWindows, tooSQL de SAP HANA, nous qu’il vous faut. 

Avec les services de machine virtuelle Microsoft Azure et SAP HANA sur de grandes instances Azure, Microsoft propose une plateforme complète sous forme d’infrastructure en tant que services (IaaS). Étant donné que la plupart des solutions SAP sont prises en charge sur Azure, ce document de prise en main sert de table des matières pour nos documents SAP actuels. Comme d’autres titres sont ajoutés bibliothèque de documents tooour - vous les voyez ajoutés ici. 
...
| --- | --- | --- |
| SAP HANA Developer Edition (y compris les logiciels de client hello HANA constitué de SQLODBC, ODBO-Windows uniquement, ODBC, JDBC Driver, HANA studio et la base de données HANA) |Red Hat Enterprise Linux, SUSE Linux Enterprise |A7, A8 |
...
Microsoft Azure est certifié pour hello suivant produits SAP, avec prise en charge complète de Microsoft et SAP.

| Produit SAP | SE invité | SGBDR | Types de machine virtuelle |
| --- | --- | --- | --- |
| SAP Business Suite Software |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (Windows uniquement), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP Business All-in-One |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (Windows uniquement), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP BusinessObjects BI |Windows |N/A |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP NetWeaver |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (Windows uniquement), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
...
Résumé : Cet article décrit comment toodeploy SAP IDE en cours d’exécution avec SQL Server et le système d’exploitation Windows sur Microsoft Azure via SAP Cloud Appliance bibliothèque 3.0. 
...
titre : aaaSAP NetWeaver sur Azure des Machines virtuelles (VM) – Guide de planification et implémentation
...
titre : aaaSAP NetWeaver sur Azure des Machines virtuelles (VM) – Guide de déploiement
...
titre : aaaSAP NetWeaver sur Azure des Machines virtuelles (VM) – Guide de déploiement de système SGBD
...
titre : aaaSAP NetWeaver sur Azure des Machines virtuelles (VM) – Guide de déploiement de haute disponibilité

Résumé : Ce document décrit comment SAP seul point de composants de défaillance comme hello ASCS/SCS de SAP et SGBD peut être protégé dans Azure. Composants de hello SAP ASCS/SCS, SGBD et application de serveurs sont essentiels pour la fonctionnalité de hello de systèmes SAP NetWeaver, par exemple, ABAP SAP NetWeaver, Java de SAP NetWeaver, les systèmes SAP NetWeaver ABAP + Java. Par conséquent, les besoins de fonctionnalités de haute disponibilité toobe placer dans placer toomake assurer que ces composants peuvent supporter la défaillance d’un serveur ou une machine virtuelle comme étant effectués avec des configurations de Cluster Windows pour complets et les environnements Hyper-V.
