...
Plusieurs ordinateurs virtuels peuvent être créés dans hello même service cloud. toounderstand ce virtual machines scénario, consultez [comment virtuels tooconnect avec un service cloud ou réseau virtuel](../classic/connect-vms.md#connect-vms-in-a-standalone-cloud-service). Ce scénario est lorsqu’un client sur une machine virtuelle tente tooconnect tooSQL serveur s’exécutant sur un autre ordinateur virtuel Bonjour même service cloud.
...
> [!NOTE]
> Il est important de toonote lorsque vous utilisez cette technique toocommunicate avec SQL Server, toutes les données sortantes à partir de hello centre de données Azure est sujet toonormal [tarification des transferts de données sortantes](https://azure.microsoft.com/pricing/details/data-transfers/).
> 
> 
...
Réseaux virtuels permet également de vous toojoin votre domaine tooa de machines virtuelles Azure. Il s’agit de hello seule façon toouse l’authentification Windows tooSQL Server. Hello autres scénarios de connexion requièrent l’authentification SQL avec des noms d’utilisateur et mots de passe.

Si vous vous apprêtez tooconfigure un environnement de domaine et l’authentification Windows, il est inutile toouse hello étapes de cet article tooconfigure hello public point de terminaison ou hello l’authentification SQL et de connexions. Dans ce scénario, vous pouvez connecter l’instance de SQL Server tooyour en spécifiant le nom de machine virtuelle SQL Server hello dans la chaîne de connexion hello. Bonjour à l’exemple suivant suppose que l’authentification Windows a été également configurée et que l’utilisateur hello a reçu l’instance de SQL Server toohello access.
...
Avant de pouvoir connecter l’instance toohello de SQL Server à partir d’une autre machine virtuelle ou hello internet, vous devez effectuer hello tâches suivantes, comme décrit dans les sections hello qui suivent :
...
Il est important tooreview tous de la sécurité de hello meilleures pratiques pour SQL Server s’exécutant sur une machine virtuelle Azure. Pour plus d’informations, consultez [Considérations relatives à la sécurité de SQL Server sur les machines virtuelles Azure](../sql/virtual-machines-windows-sql-security.md).
