...
Découvrez comment des travaux toouse Azure PowerShell toorun Sqoop dans HDInsight tooimport et d’exportation entre le cluster HDInsight et de la base de données SQL Azure ou base de données SQL Server.

> [!NOTE]
> étapes Hello dans cet article peuvent être utilisés avec soit un cluster de HDInsight Windows ou Linux. Toutefois, ces étapes fonctionne uniquement à partir d’un client Windows. Pour les autres méthodes de soumission de travail, cliquez sur le sélecteur de tabulations hello sur haut hello du hello.
> 
> 
...
Bonjour suite du script PowerShell prétraite le fichier de source de hello et il exporte la base de données SQL Azure tooan :
