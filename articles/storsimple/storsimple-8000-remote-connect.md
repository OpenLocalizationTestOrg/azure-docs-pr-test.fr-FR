...
Vous pouvez vous connecter à distance appareil tooyour via Windows PowerShell. Quand vous vous connectez de cette façon, aucun menu ne s’affiche. (Vous consultez un menu uniquement si vous utilisez la console série de hello sur hello appareil tooconnect.) Avec accès à distance de Windows PowerShell, vous vous connectez tooa les instance d’exécution spécifique. Vous pouvez également spécifier la langue d’affichage hello.

Pour plus d’informations sur l’utilisation de Windows PowerShell remoting toomanage votre appareil, consultez trop[utiliser Windows PowerShell pour StorSimple tooadminister votre appareil StorSimple](storsimple-8000-windows-powershell-administration.md).

Ce didacticiel explique comment tooconfigure votre appareil pour l’administration à distance et puis tooconnect tooWindows PowerShell pour StorSimple. Vous pouvez utiliser HTTP ou HTTPS tooremotely se connecter via Windows PowerShell. Toutefois, lorsque vous décidez comment tooconnect tooWindows PowerShell pour StorSimple, envisagez de hello informations suivantes :
...
Vous pouvez vous connecter à distance interface Windows PowerShell de toohello. Cependant, l’appareil StorSimple tooyour accès à distance via l’interface Windows PowerShell de hello n’est pas activée par défaut. Vous devez tout d’abord activer la gestion à distance sur un périphérique de hello et puis sur hello client qui est utilisé tooaccess votre appareil.
...
Vous pouvez utiliser soit hello Azure portal ou hello console série tooconfigure gestion à distance. Sélectionnez hello procédures suivantes :

* [Utiliser la gestion à distance de tooenable portail Azure hello sur HTTP](#use-the-azure-classic-portal-to-enable-remote-management-over-http)
...
### <a name="use-hello-azure-portal-tooenable-remote-management-over-http"></a>Utiliser la gestion à distance de tooenable portail Azure hello sur HTTP

Effectuer hello suivant les étapes de gestion à distance de hello tooenable portail Azure via HTTP.

#### <a name="tooenable-remote-management-through-hello-azure-portal"></a>tooenable la gestion à distance via hello portail Azure

1. Atteindre le service du Gestionnaire de périphériques StorSimple tooyour. Sélectionnez **périphériques** puis sélectionnez et cliquez sur hello périphérique tooconfigure pour la gestion à distance. Accédez trop**paramètres du périphérique > sécurité**.
2. Bonjour **paramètres de sécurité** panneau, cliquez sur **gestion à distance**.
3. Bonjour **gestion à distance** panneau, définissez **activer la gestion à distance** trop**Oui**.
...
3. Vous êtes notifié des failles de sécurité hello de l’unité de toohello tooconnect HTTP. Quand vous y êtes invité, confirmez en tapant **O**.
...
* [Utilisez la gestion à distance de hello tooenable portail Azure via le protocole HTTPS](#use-the-azure-classic-portal-to-enable-remote-management-over-https)
...
### <a name="use-hello-azure-portal-tooenable-remote-management-over-https"></a>Utilisez la gestion à distance de hello tooenable portail Azure via le protocole HTTPS

Effectuer hello suivant les étapes de gestion à distance de hello tooenable portail Azure via le protocole HTTPS.

#### <a name="tooenable-remote-management-over-https-from-hello-azure-portal"></a>tooenable de gestion à distance via HTTPS à partir de hello portail Azure
...
5. Cliquez sur ... puis sur **Télécharger le certificat de gestion à distance**. Spécifiez un emplacement toosave ce fichier. Vous devez tooinstall ce certificat sur l’ordinateur client ou l’hôte de hello que vous allez utiliser tooconnect toohello appareil.
...
Chacun des hello précédant les procédures, est décrite ci-dessous.

#### <a name="tooimport-hello-certificate-on-hello-remote-host"></a>certificat de hello tooimport sur l’hôte distant de hello
1. Avec le bouton droit de fichier .cer hello et sélectionnez **installer le certificat**. Cette commande démarre hello Assistant Importation de certificat.
...
2. Ajoutez hello appareil IP adresse toohello hôtes approuvés du client en tapant :
    ...
    Où <*device_ip*> est l’adresse IP de hello de votre appareil, par exemple : 
     ...
     `Set-Item wsman:\localhost\Client\TrustedHosts 10.126.173.90 -Concatenate -Force`
3. toocreate une information d’identification, tapez :
...
6. Vous devez toowait quelques minutes, et vous serez ensuite appareil tooyour connecté via HTTPS sur SSL. Vous voyez un message indiquant que vous êtes connecté tooyour appareil.
...
* En savoir plus sur [votre appareil StorSimple à l’aide de Windows PowerShell tooadminister](storsimple-8000-windows-powershell-administration.md).
