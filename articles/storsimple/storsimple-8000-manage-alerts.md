...
---
# <a name="use-hello-storsimple-device-manager-service-tooview-and-manage-storsimple-alerts"></a>Utilisez tooview de service de gestionnaire de périphériques StorSimple hello et gérer les alertes StorSimple

## <a name="overview"></a>Vue d'ensemble

Hello **alertes** panneau Bonjour le Gestionnaire de périphériques StorSimple service offre un moyen pour vous tooreview et désactivez les alertes StorSimple liée à un appareil en temps réel. À partir de ce panneau, vous pouvez centraliser surveiller les problèmes de contrôle d’intégrité hello de vos appareils StorSimple et hello solution globale de Microsoft Azure StorSimple.
...
1. Atteindre le service du Gestionnaire de périphériques StorSimple tooyour. À partir de la liste de hello des appareils, sélectionnez et cliquez sur périphérique hello que vous souhaitez tooconfigure.
2. Accédez trop**paramètres** > **général** pour appareil de hello.

   ![Panneau Alertes](./media/storsimple-8000-manage-alerts/configure-alerts-email2.png)
   
2. Bonjour **paramètres généraux** panneau, accédez trop**paramètres d’alerte** et définir hello :
   
   1. Bonjour **envoyer une notification par courrier électronique** champ, sélectionnez **Oui**.
   2. Bonjour **administrateurs de service de messagerie** champ, sélectionnez **Oui** administrateur du service toohave hello et tous les coadministrateurs reçoivent des notifications d’alerte hello.
...
3. toosend une notification par courrier électronique de test, cliquez sur **envoyer par courrier électronique de test**. Hello service du Gestionnaire de périphériques StorSimple affiche des messages d’état pendant qu’il transmet la notification de test hello.

    ![Paramètres d’alerte](./media/storsimple-8000-manage-alerts/configure-alerts-email3.png)

4. Vous voyez une notification lors de l’envoi de courrier électronique de test hello. 
   
    ![E-mail de notification de test des alertes envoyé](./media/storsimple-8000-manage-alerts/configure-alerts-email4.png)
   
   > [!NOTE]
   > Si le test de hello message de notification ne peut pas envoyé, service de gestionnaire de périphériques StorSimple hello affiche un message d’erreur approprié. Attendez quelques minutes et réessayez toosend votre message de notification de test. 

5. Une fois que vous avez terminé la configuration de hello, cliquez sur **enregistrer**. Cliquez sur **Oui**lorsque vous êtes invité à confirmer l’opération.
...
panneau Résumé du service Gestionnaire de périphériques StorSimple Hello vous donne un coup de œil au numéro hello d’alertes sur vos appareils, organisés par niveau de gravité.

![Tableau de bord des alertes](./media/storsimple-8000-manage-alerts/device-summary4.png)

Cliquez sur un niveau de gravité hello ouvre hello **alertes** panneau. les résultats de Hello incluent uniquement les alertes de hello qui correspondent à ce niveau de gravité.
...
| Impossible de connecter le service d’authentification toohello. |Erreur de chemin d’accès des données |Hello URLthat sert tooauthenticate n’est pas accessible. Vérifiez que vos règles de pare-feu incluent des modèles d’URL spécifiés pour l’appareil StorSimple hello hello. Pour plus d’informations sur les modèles d’URL dans le portail Azure, accédez à toohttps://aka.ms/ss-8000-network-reqs. Si vous utilisez le Cloud Azure gouvernement, accédez à des modèles d’URL toohello dans https://aka.ms/ss8000-gov-network-reqs.|

### <a name="performance-alerts"></a>Alertes de performances

| Texte d'alerte | Événement | Plus d'informations/actions recommandées |
|:--- |:--- |:--- |
| Hello charge de l’appareil a dépassé <*seuil*>. |Plus lent que le temps de réponse attendu. |Votre appareil indique que l'utilisation d'entrée/sortie est surchargée. Cela peut entraîner votre travail de toonot appareil ainsi comme il le devrait. Passez en revue les charges de travail hello que vous avez connecté toohello périphérique et déterminez s’il existe un qui peut être déplacée tooanother appareil ou qui ne sont plus nécessaires.| Impossible de démarrer les services StorSimple. |Erreur de chemin d’accès des données |Si hello problème persiste, contactez le Support Microsoft. |et état actuel de hello, accédez trop[utilisation hello toomonitor du service Gestionnaire de périphériques StorSimple votre appareil](storsimple-monitor-device.md) |
