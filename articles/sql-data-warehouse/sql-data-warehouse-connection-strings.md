...
Vous pouvez vous connecter tooSQL l’entrepôt de données avec plusieurs protocoles d’application différents, tels que [ADO.NET][ADO.NET], [ODBC][ODBC], [PHP] [ PHP] et [JDBC][JDBC]. Voici quelques exemples de chaînes de connexion pour chaque protocole.  Vous pouvez également utiliser hello toobuild portail Azure à votre chaîne de connexion.  toobuild votre chaîne de connexion à l’aide de hello le portail Azure, accédez tooyour Panneau de la base de données, sous *Essentials* cliquez sur *afficher les chaînes de connexion de base de données*.
...
> [!NOTE]
> Envisagez le paramètre hello connexion délai too300 (secondes) dans l’ordre tooallow hello connexion toosurvive courtes périodes d’indisponibilité.
> 
> 
