...
> 
 ...
 Ce didacticiel vous montre comment toocreate un pipeline de données Azure Data Factory toomove tooSQL de l’objet Blob de stockage Azure Data Warehouse. Avec hello, vous allez comme suit :
...
Une fois que tous les éléments hello prêt, vous êtes prêt toocopy exemple tooyour de données objet Blob de stockage Azure.
...
1. Maintenant, nous allons définir notre jeu de données pour SQL Data Warehouse.  Nous allons commencer Bonjour même manière, en cliquant sur « Nouveau jeu de données », puis sur Azure SQL Data Warehouse.
...
Enfin, nous allons configuration et exécution du pipeline hello dans Azure Data Factory.  Il s’agit d’opération hello qui s’achève le déplacement des données réelles de hello.  Vous trouverez une vue complète des opérations hello que vous pouvez réaliser avec SQL Data Warehouse et Azure Data Factory [ici][Move data tooand from Azure SQL Data Warehouse using Azure Data Factory].

Section de 'Auteur et déployer' hello cliquez maintenant sur « Plus les commandes », puis « Nouveau Pipeline ».  Après avoir créé le pipeline de hello, vous pouvez utiliser hello ci-dessous code tootransfer hello données tooyour data warehouse :
...
* [Didacticiel : Prise en main Azure Data Factory] [ Tutorial: Get started with Azure Data Factory] didacticiel de base hello pour le traitement des données avec Azure Data Factory. Dans ce didacticiel, vous allez créer votre premier pipeline qui utilise HDInsight tootransform puis analyser les journaux web sur une base mensuelle. Notez que ce didacticiel ne couvre aucune activité de copie.
* [Didacticiel : Copier des données à partir de l’objet Blob de stockage Azure tooAzure base de données SQL][Tutorial: Copy data from Azure Storage Blob tooAzure SQL Database]. Dans ce didacticiel, vous allez créer un pipeline de données de toocopy Azure Data Factory à partir de l’objet Blob de stockage Azure tooAzure base de données SQL.
