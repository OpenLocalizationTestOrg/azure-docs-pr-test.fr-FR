...
-   Avant de créer un travail d’importation, vous pouvez utiliser cet outil toocopy données toohello disques durs que vous allez centre de données tooship tooa Windows Azure.  
...
Si vous préparez des disques pour un travail d’importation, vous devez hello toomeet suivant des conditions préalables :  
...
-   Vous devez au moins une des clés de compte hello hello compte de stockage.  
...
-   Vous aurez besoin d’un ou plusieurs disques contenant toobe de données importé ou vides disques durs SATA de 3,5 pouces connectés ordinateur de copie toohello.  
...
Si vous essayez de toorepair une importation qui a partiellement échoué, vous devez :  
  
-   fichiers journaux de copie Hello  
  
-   clé de compte de stockage Hello  
  
  Si vous essayez de toorepair une exportation qui a partiellement échoué, vous devez :  
...
-   Hddid.dll  
 ...
 Copiez ces répertoire de travail tooa de fichiers, par exemple, `c:\WAImportExport`. Ensuite, ouvrez une fenêtre de ligne de commande en mode administrateur et définissez les hello du répertoire en tant que répertoire actif.  
  
 toooutput l’aide pour la commande hello, exécutez l’outil hello sans paramètres :  
