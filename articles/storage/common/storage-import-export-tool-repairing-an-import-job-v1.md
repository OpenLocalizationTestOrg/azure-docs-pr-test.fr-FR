...
Vous pouvez exécuter hello Microsoft Azure Import/Export Tool avec du travail d’importation hello copie journal des fichiers et hello outil téléchargements hello fichiers manquants (ou parties d’un fichier) tooyour Windows Azure storage compte toocomplete hello travail d’importation.  
...
|**/CopyLogFile:**&lt;DriveCopyLogFile\>|**Obligatoire.** Chemin d’accès toohello lecteur fichier journal de copie (journal détaillé ou journal des erreurs). fichier de Hello est généré par hello service Windows Azure Import/Export et peut être téléchargé à partir du stockage d’objets blob hello associé hello travail. fichier journal de copie Hello contient des informations sur les objets BLOB en échec ou aux fichiers qui sont toobe réparé.|  
|**/PathMapFile:**&lt;DrivePathMapFile\>|**Facultatif.** Fichier texte tooa chemin d’accès peut être utilisé tooresolve ambiguïtés si vous avez plusieurs fichiers hello même nom que celui que vous importez dans hello même travail. outil hello première Hello est exécutée, elle peut remplir ce fichier avec tous les noms ambigus hello. Les exécutions suivantes de l’outil de hello utilisent cette ambiguïtés au niveau du fichier tooresolve hello.|  
...
Hello `/PathMapFile` option vous permet de tooresolve ces erreurs. Vous pouvez spécifier le nom hello du fichier hello, qui contient la liste hello des fichiers hello outil n’a pas pu identifier de toocorrectly. Hello exemple de ligne de commande suivant remplit `9WM35C2V_pathmap.txt`:  
