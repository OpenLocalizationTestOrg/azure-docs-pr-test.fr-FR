...
Dans ce didacticiel, vous apprendrez comment toointegrate Symantec Web sécurité Service (WSS) avec Azure Active Directory (Azure AD).

Intégration de Service de sécurité Web Symantec (WSS) avec Azure AD offre hello avantages suivants :

- Vous pouvez contrôler dans Azure AD qui a accès tooSymantec sécurité Service WSS (Web)
- Vous pouvez activer vos utilisateurs tooautomatically get connecté tooSymantec Web sécurité Service (WSS) (Single Sign-On) avec leurs comptes Azure AD
...
1. Ajout de Service de sécurité Web Symantec (WSS) à partir de la galerie de hello
...
4. Dans la zone de recherche de hello, tapez **Symantec Web sécurité Service (WSS)**.
...
5. Dans le volet de résultats hello, sélectionnez **Symantec Web sécurité Service (WSS)**, puis cliquez sur **ajouter** bouton application hello de tooadd.
...
3. **[Création d’un utilisateur de test du Service de sécurité Web Symantec (WSS)](#creating-a-symantec-web-security-service-wss-test-user)**  -toohave un équivalent de Britta Simon dans Symantec Security Service WSS (Web) qui est la représentation sous forme de toohello lié Azure AD de l’utilisateur.
...
3. Sur hello **URL et le domaine de Service (WSS) de sécurité Web Symantec** section, effectuer hello comme suit :
    ...
    a. Bonjour **URL de connexion** zone de texte, url mention hello, que vous souhaitez tooblock selon la stratégie de blocage toohello Hello Symantec Security Service WSS (Web).  
    
    b. Bonjour **identificateur** zone de texte, tapez l’URL hello :`https://saml.threatpulse.net:8443/saml/saml_realm`

    > [!NOTE] 
    > Il ne s’agit pas de valeurs réelles. Mettre à jour les valeurs de hello réel Sign-On URL et l’identificateur. Contact [équipe de support Client de Service de sécurité Web Symantec (WSS)](https://www.symantec.com/contact-us) tooget ces valeurs. 
...
6. tooconfigure l’authentification unique sur **Symantec Web sécurité Service (WSS)** côté, vous devez hello toosend téléchargé **Metadata XML** trop[équipedesupportdeSymantecSecurityServiceWSS(Web)](https://www.symantec.com/contact-us).
...
Dans cette section, vous créez un utilisateur appelé Britta Simon dans Symantec Web Security Service (WSS). Travailler avec [équipe de support du Service de sécurité Web Symantec (WSS)](https://www.symantec.com/contact-us) pour ajouter des utilisateurs de hello de plateforme de Service de sécurité Web Symantec (WSS) hello. Les utilisateurs doivent être créés et activés avant que vous utilisiez l’authentification unique. En même temps que hello utilisateur détaille également vous devez toosend hello adresse IP publique de l’ordinateur hello à partir de laquelle vous essayez d’application de tooaccess hello.
...
Lorsque vous cliquez sur hello Symantec Web sécurité Service (WSS) vignette Bonjour volet d’accès, vous obtenez redirigé toohello blocage de page pour laquelle hello stratégie de blocage a été configuré.
