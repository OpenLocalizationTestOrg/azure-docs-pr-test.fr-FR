...
L’objectif de ce didacticiel est de vous montrer les étapes à effectuer dans LinkedIn Lookup et Azure AD pour approvisionner et retirer automatiquement des comptes d’utilisateur d’Azure AD vers LinkedIn Lookup. 
...
*   Un compte Administrateur dans LinkedIn Lookup ayant accès au Centre des comptes LinkedIn

> [!NOTE]
> Azure Active Directory s’intègre dans LinkedIn Lookup à l’aide du protocole [SCIM](http://www.simplecloud.info/).

## <a name="assigning-users-to-linkedin-lookup"></a>Assignation d’utilisateurs à LinkedIn Lookup
...
### <a name="important-tips-for-assigning-users-to-linkedin-lookup"></a>Conseils importants pour l’assignation d’utilisateurs à LinkedIn Lookup

*   Il est recommandé d’assigner un seul utilisateur Azure AD à LinkedIn Lookup pour tester la configuration de l’approvisionnement. Les autres utilisateurs et/ou groupes peuvent être affectés ultérieurement.

*   Quand vous assignez un utilisateur à LinkedIn Lookup, vous devez sélectionner le rôle **Utilisateur** dans la boîte de dialogue d’assignation. Le rôle « Accès par défaut » ne fonctionne pas pour l’approvisionnement.


## <a name="configuring-user-provisioning-to-linkedin-lookup"></a>Configuration de l’approvisionnement des utilisateurs sur LinkedIn Lookup

Cette section vous guide pour connecter votre instance d’Azure AD à l’API d’approvisionnement de comptes d’utilisateur SCIM de LinkedIn Lookup et pour configurer le service d’approvisionnement afin de créer, mettre à jour et désactiver des comptes d’utilisateur assignés dans LinkedIn Lookup en fonction des assignations d’utilisateurs et de groupes dans Azure AD.
...
### <a name="to-configure-automatic-user-account-provisioning-to-linkedin-lookup-in-azure-ad"></a>Pour configurer l’approvisionnement automatique de comptes d’utilisateur sur LinkedIn Lookup dans Azure AD :
...
8) Si vous avez déjà configuré LinkedIn Lookup pour l’authentification unique, recherchez votre instance de LinkedIn Lookup à l’aide du champ de recherche. Sinon, sélectionnez **Ajouter** puis recherchez **LinkedIn Lookup** dans la galerie d’applications. Sélectionnez LinkedIn Lookup dans les résultats de la recherche, puis ajoutez-le à votre liste d’applications.

9)  Sélectionnez votre instance de LinkedIn Lookup, puis sélectionnez l’onglet **Approvisionnement**.
...
14) Dans la section **Mappages d’attributs**, passez en revue les attributs d’utilisateur et de groupe qui seront synchronisés d’Azure AD vers LinkedIn Lookup. Les attributs sélectionnés comme propriétés **Correspondance** sont utilisés pour faire correspondre les comptes d’utilisateur et les groupes dans LinkedIn Lookup pour les opérations de mise à jour. Cliquez sur le bouton Enregistrer pour valider les modifications.

![Approvisionnement LinkedIn Lookup](./media/active-directory-saas-linkedinlookup-provisioning-tutorial/linkedin_4.PNG)

15) Afin d’activer le service d’approvisionnement Azure AD pour LinkedIn Lookup, définissez l’**État d’approvisionnement** sur **Activé** dans la section **Paramètres**

16) Cliquez sur **Save**. 

Cette commande lance la synchronisation initiale des utilisateurs et/ou groupes assignés à LinkedIn Lookup dans la section Utilisateurs et groupes. Notez que la synchronisation initiale prend plus de temps que les synchronisations suivantes, qui se produisent toutes les 20 minutes environ tant que le service est en cours d’exécution. Vous pouvez utiliser la section **Détails de la synchronisation** pour surveiller la progression et suivre les liens vers les rapports d’activité d’approvisionnement, qui décrivent toutes les actions effectuées par le service d’approvisionnement dans votre application LinkedIn Lookup.
