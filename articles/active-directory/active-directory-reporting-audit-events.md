...
Pour obtenir les informations les plus récentes sur la rétention, consultez [Stratégies de rétention des rapports Azure Active Directory](active-directory-reporting-retention.md).
...
| MailNickname |Moniker d’un objet de carnet d’adresses, généralement la partie de l’adresse e-mail précédant le symbole « @ ». |
...
| DirSyncEnabled |Indique si la synchronisation se produit à partir d’un annuaire faisant autorité (client, local). |
| DisplayName |Nom d’affichage d’un objet. |
| IsCompliant |Cet attribut est utilisé pour gérer l’état de l’appareil lié à la gestion des appareils mobiles. |
