...
Toutes les fonctionnalités de basée sur le groupe de licences sont disponible via hello [portail Azure](https://portal.azure.com), et actuellement prise en charge de PowerShell est limité. Toutefois, il existe de certaines tâches utiles qui peuvent être effectuées à l’aide de hello existant [applets de commande PowerShell de MSOnline](https://docs.microsoft.com/powershell/msonline/v1/azureactivedirectory). Ce document fournit des exemples de ce qui est possible.

> [!NOTE]
> Avant de commencer, les applets de commande en cours d’exécution, assurez-vous que vous vous connectez en premier lieu, tooyour client en exécutant hello `Connect-MsolService` applet de commande.

## <a name="view-product-licenses-assigned-tooa-group"></a>Tooa groupe affecté à afficher les licences de produit
Le [Get-MsolGroup](/powershell/module/msonline/get-msolgroup?view=azureadps-1.0) applet de commande peut être un objet de groupe utilisés tooretrieve hello et vérifier les hello *licences* propriété : il répertorie toutes les licences du produit actuellement affecté toohello groupe.
```
...
> [!NOTE]
> les données de salutation sont limitée tooproduct (référence SKU) des informations. Il n’est pas désactivées dans la licence de hello des plans de services possible toolist hello.

## <a name="get-all-groups-with-licenses"></a>Obtenir tous les groupes avec des licences

Vous pouvez trouver tous les groupes avec une licence attribuée par en cours d’exécution hello commande suivante :
```
...
Vous pouvez rapporter des statistiques de base sur les groupes avec des licences. Nombre d’utilisateurs total hello nombre hello d’utilisateurs dont les licences déjà affectés au groupe de hello et le nombre de hello d’utilisateurs pour lesquels des licences n’a pas pu être attribués par groupe de hello sont répertoriées dans l’exemple hello ci-dessous.
...
toofind les groupes qui contiennent des utilisateurs dont les licences ne peut pas être attribués :
```
...
```
## <a name="get-all-users-with-license-errors-in-hello-entire-tenant"></a>Obtenir tous les utilisateurs avec des erreurs de licence dans le locataire entière de hello

toolist tous les utilisateurs qui ont des erreurs de licence à partir d’un ou plusieurs groupes, hello script suivant peuvent être utilisés. Ce script affiche une ligne par utilisateur, par erreur de licence qui vous permet de tooclearly identifier source hello de chaque erreur.

> [!NOTE]
> Ce script énumère tous les utilisateurs de locataire hello, ce qui peut ne pas être optimale pour les clients de grande taille.
...
Voici une autre version de script hello qui effectue une recherche uniquement dans les groupes qui contiennent des erreurs de licence. Il peut être que plus optimisé pour les scénarios où vous prévoyez toohave plusieurs groupes avec des problèmes.
...
Pour un objet utilisateur, il est possible de toocheck si une licence de produit spécifique est affectée à partir d’un groupe ou si elle est directement affectée.

Hello deux exemples de fonctions ci-dessous peuvent être utilisé de type hello de tooanalyze d’affectation sur un utilisateur individuel :
```
...
Ce script s’exécute ces fonctions sur chaque utilisateur de client hello, en tant qu’entrée à l’aide de hello ID de référence (SKU) : dans cet exemple nous intéresse licence hello pour *Enterprise Mobility + Security*, ce qui, dans notre client est représenté avec l’ID *contoso:EMS*:
```
...
Hello ce script vise tooremove inutiles direct licences aux utilisateurs qui héritent déjà hello même de licence à partir d’un groupe ; par exemple, dans le cadre d’un [transition licence toogroup](https://docs.microsoft.com/azure/active-directory/active-directory-licensing-group-migration-azure-portal).
> [!NOTE]
> Il est important de toofirst valider ce hello toobe direct licences supprimée n’activez pas plus de fonctionnalités de service qu’hello héritée de licences. Dans le cas contraire, suppression des licences de direct hello peut désactiver tooservices d’accès et des données pour les utilisateurs. Actuellement, il n’est pas possible toocheck via PowerShell quels services sont activés via vs héritées licences directes. Dans le script de hello, nous spécifions minimaux services nous savons hello sont héritées de groupes et nous allons vérifier par rapport à qui.
...
toolearn plus d’informations sur la fonctionnalité de hello définie pour la gestion des licences via des groupes, voir hello :
