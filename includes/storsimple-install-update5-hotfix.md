...
3. Dans la zone de recherche hello Hello catalogue Microsoft Update, entrez le nombre de Base de connaissances (KB) hello de hello correctif souhaité toodownload, par exemple **4037264**, puis cliquez sur **recherche**.
   
    Hello correctif liste s’affiche, par exemple, **Cumulative 5.0 de mise à jour logicielle offre groupée pour StorSimple 8000 Series**.
...
5. Recherche de tous les correctifs supplémentaires répertoriées dans le tableau hello ci-dessus (**4037266**), et hello de téléchargement correspondant de fichiers des dossiers spécifiques toohello comme indiqué dans le tableau précédent de hello.
...
4. Fournissez le mot de passe hello lorsque vous y êtes invité. Vous trouverez ci-dessous un exemple de sortie pour l’installation des mises à jour de commande premier hello. Hello première commande mise à jour, vous devez toopoint toohello un fichier spécifique.

    >[!NOTE] 
    > Vous devez installer hello _HcsSoftwareUpdate.exe_ premier. Une fois l’installation terminée, installez le fichier _CisMdsAgentUpdate.exe_.
        ...
        ````
5. Type **Y** lorsque tooconfirm demandée hello installation du correctif logiciel.
6. Surveiller les mises à jour hello à l’aide de hello `Get-HcsUpdateStatus` applet de commande. mise à jour Hello terminera tout d’abord sur le contrôleur passif de hello. Une fois que le contrôleur passif de hello est mise à jour, il y aura un basculement et mise à jour hello puis appliqué sur hello autre contrôleur. mise à jour Hello est terminée lorsque les deux contrôleurs hello sont mis à jour.
   
    Hello résultat de l’exemple suivant illustre hello mise à jour en cours d’exécution. Hello `RunInprogress` est `True` lorsque la mise à jour hello est en cours d’exécution.
    ...
    ```
     ...
     Hello suivant l’exemple de sortie indique que cette mise à jour hello est terminée. Hello `RunInProgress` est `False` lorsque la mise à jour hello est terminée.
...
7. Une fois la mise à jour logicielle de hello est terminée, vérifiez les versions des logiciels système hello. Entrez :
   ...
   *  `HcsSoftwareVersion: 6.3.9600.17845`
    ...
    Si le numéro de version hello ne change pas après avoir appliqué la mise à jour hello, il indique ce correctif hello a échoué tooapply. Dans ce cas, contactez le [Support Microsoft](../articles/storsimple/storsimple-8000-contact-microsoft-support.md) pour obtenir une assistance supplémentaire.
...
8. Répétez les étapes 3 à 6 tooinstall hello _CisMDSAgentupdate.exe_ agent téléchargé tooyour _FirstOrderUpdate_ dossier.
8. Répétez les étapes 3 à 6 tooinstall hello deuxième mises à jour. 

    > [!NOTE] 
    > Deuxième mise à jour de l’ordre, plusieurs mises à jour peuvent être installés en exécutant simplement les hello `Start-HcsHotfix cmdlet` et pointant vers le dossier toohello où se trouvent le deuxième mises à jour. applet de commande Hello s’exécute toutes les mises à jour de hello disponibles dans le dossier de hello. Si une mise à jour est déjà installé, la logique de mise à jour hello détectera qu’et pas appliquer cette mise à jour.
...
Utilisez les mises à jour du microprogramme KB4037263 tooinstall disque. Ces mises à jour et prennent environ 30 minutes toocomplete. Vous pouvez choisir tooinstall dans une fenêtre de maintenance planifiée par la console série du périphérique toohello connexion.

> [!NOTE] 
> Si le microprogramme du disque est déjà à jour, vous n’aurez pas tooinstall ces mises à jour. Exécutez hello `Get-HcsUpdateAvailability` applet de commande hello appareil console série toocheck si les mises à jour sont disponibles et hello si des mises à jour sont perturbateur (mode de maintenance) ou sans interruption de service (mode standard) des mises à jour.

tooinstall hello disque mises à jour, suivez les instructions de hello ci-dessous.

1. Placez l’appareil de hello en mode de maintenance hello. 

    > [!NOTE] 
    > N’utilisez pas la communication à distance de Windows PowerShell lors de la connexion du périphérique tooa en mode maintenance. Au lieu de cela, exécutez cette applet de commande sur le contrôleur de l’appareil hello lorsqu’il est connecté via la console série du périphérique hello.

    contrôleur de hello tooplace en mode maintenance, tapez :
...
5. contrôleurs de Hello redémarrer lorsque vous quittez le mode maintenance. Une fois que du microprogramme du disque hello mises à jour sont correctement appliquées et appareil de hello a quitté le mode de maintenance, retour toohello portail Azure. Notez que ce portail hello peut ne pas affiche que vous avez installé des mises à jour du mode de maintenance hello pendant 24 heures.
